...
def _get_work_dir():
    ...
    try:
        ...
        return config.get('Settings', 'work_dir', fallback='test')
    except:
        return "test"
...
class App:
    ...
    def build_ui(self):
        ...
        # 第一行按钮
        tk.Button(btn_frame, text="🔍 账号转UID", **btn1, command=self.get_uid_thread).grid(row=0, column=0, padx=5, pady=5)
        tk.Button(btn_frame, text="⬇ 指定UID下载", **btn2, command=self.start_single).grid(row=0, column=1, padx=5, pady=5)
        
        # 第二行按钮
        tk.Button(btn_frame, text="📂 导入UID文件", **btn3, command=self.load_uid_file).grid(row=1, column=0, padx=5, pady=5)
        tk.Button(btn_frame, text="🚀 批量下载", **btn4, command=self.start_async_batch).grid(row=1, column=1, padx=5, pady=5)
    ...
    def load_uid_file(self):
        ...
        try:
            ...
            self.log(f"📂 导入 {len(self.uid_list)} 个UID (文件: {os.path.basename(path)})")
    ...
    async def async_batch_task(self):
        ...
        if self.batch_file_path:
            file_name = os.path.splitext(os.path.basename(self.batch_file_path))[0]
            output_folder_name = file_name
            self.log(f"📁 输出文件夹: {self.archiver}{output_folder_name}/")

        tasks = []
        for uid in uids:
            if output_folder_name:
                original_save_dir = BASE_SAVE_DIR
                new_save_dir = os.path.join(original_save_dir, output_folder_name)
                tasks.append(self.download_with_custom_dir(uid, new_save_dir))
            else:
    ...
    async def download_with_custom_dir(self, uid, custom_dir):
        """使用自定义目录下载单个UID的所有存档"""
        gamekey = self.archiver.calculate_gamekey()
        uid_folder = os.path.join(custom_dir, uid)
        os.makedirs(uid_folder, exist_ok=True)
    
        self.log(f"[{uid}] 开始下载")

        results = []
        tasks = [self.archiver.download_single_slot(str(i), uid, gamekey, uid_folder) for i in range(8)]
        raw_results = await asyncio.gather(*tasks)
