...
class DetectionEngine:
    ...
    def check_vip_pay_union(file_path, pay_cost):
        ...
        if ratio2 > 1.0 and vip_level < 10:
            status = 'fail'
            msg_lines.append(f"VIP{vip_level}消耗：{pay_cost}黄金")
        elif ratio2 > 1.0 and vip_level == 10:
            status = 'warn'
            msg_lines.append(f"VIP{vip_level}消耗：{pay_cost}黄金")
        elif ratio1 < 0.3:
            status = 'warn'
            msg_lines.append(f"VIP{vip_level}消耗：{limit_amount1}\n可能出现覆盖存档")
        else:
            ...
            if check_reason:
                if reason_content is not None:
                    ...
                    if clean_reason == "":
                        ...
                        results.append({'title': "📝 ZuobiReason 分析", 'status': 'fail', 'msg': f"发现作弊记录:{display}"})
                else:
    ...
    def check_uid_md5_advanced(file_path):
        try:
            ...
            if not tag_md5:
                return {'status': 'warn', 'msg': '⚠️ 未找到数字签名', 'uid': None}

            target_md5 = tag_md5.group(1).strip().lower()
            if not target_md5:
                return {'status': 'fail', 'msg': '❌ 数字签名为空', 'uid': None}
...
class AppDetector:
    def __init__(self, root):
        self.root = root
        self.root.title("数据检测工具 2.β|BY. 观星者")
    ...
    def _build_ui(self):
        ...
        tk.Label(nav_frame, text="数据检测工具 2.0.0", bg=self.COLOR_NAV_BG, fg="white", font=("Microsoft YaHei", 12, "bold")).place(relx=0.8, y=0)
    ...
    def _run_info_get_v2(self):
        try:
            import threading
            import info_get_v2
            
            t = threading.Thread(target=info_get_v2.run_tool, daemon=True)
            t.start()
            
            self.log("下载工具已启动", "INFO")

        except ImportError:
            self.log("错误：找不到 info_get_v2 模块，请确认已打包", "FAIL")
        except Exception as e:
    ...
    def _build_home_page(self):
        ...
        tk.Button(left_frame, text="📄 打开下载工具", command=self._run_info_get_v2,bg="#E0E0E0", fg="#EE0B0B", font=("Microsoft YaHei", 10), bd=1, relief="solid").place(x=20, y=btn_y_3, width=(left_width - 40), height=btn_h)
...
"""
    ...
    def _do_batch_detect(self, folder_path):
        ...
        with ThreadPoolExecutor(max_workers=thread_count) as executor:
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    self.log(f"文件:{os.path.basename(fp)} | UID:{res['uid']} | 状态:{status}", status)
        ...
        uid_groups = defaultdict(lambda: { 'results': [], 'ban_reasons': [] })
        for file_path, res in file_result_map.items():
            uid_groups[res['uid']]['results'].append((file_path, res))

        for uid, reasons in SLOT_BAN_CACHE.ban_cache.items():
            if not reasons:
                continue
            if isinstance(reasons, list):
                cleaned = [r for r in reasons if r]
                if cleaned:
                    unique_reasons = list(dict.fromkeys(cleaned))
                    uid_groups[uid]['ban_reasons'].extend(unique_reasons)
            else:
                uid_groups[uid]['ban_reasons'].append(str(reasons))

        abnormal_report = []  # 用于CSV报告的行数据
        ...
        for uid, group in uid_groups.items():
            ...
            for ban_reason in group['ban_reasons']:
                if ban_reason and ban_reason not in seen_msgs:
                    seen_msgs.add(ban_reason)
                    detail_lines.append(f"--- [封禁异常] ---\n{ban_reason}\n")
                    combined_fail_msgs.append(ban_reason)

            for file_path, res in group['results']:
                fname = os.path.basename(file_path)
                detail_lines.append(f"--- [文件 {fname}] ---\n")
                ...
                for item in res['results']:
                    ...
                    msg = item.get('msg', '')
                    detail_lines.append(f"【{title}】 [{status}]\n{msg}\n")

                    if status == 'fail':
                        short_msg = msg.split('\n')[0] if msg else ""
                        if len(short_msg) > 100:
                            short_msg = short_msg[:100] + "..."
                        if short_msg and short_msg not in seen_msgs:
            ...
            if combined_fail_msgs:
                uid_status = 'fail'
                uid_reason = "；".join(combined_fail_msgs)
            elif combined_warn_msgs:
                uid_status = 'warn'
                uid_reason = "；".join(combined_warn_msgs)
            else:
                continue

            safe_reason = uid_reason.replace('\n', ' ').replace('\r', ' ').strip()
            if len(safe_reason) > 500:
                safe_reason = safe_reason[:500] + "..."

            detail_filename = f"{uid}_异常详情.txt"
            detail_path = os.path.join(detail_dir, detail_filename)
            with open(detail_path, "w", encoding="utf-8") as f:
                f.write(f"UID：{uid}\n")
                f.write(f"状态：{uid_status}\n")
                f.write("\n".join(detail_lines))

            if uid_status == 'fail':
                abnormal_report.append(f"❌异常：{uid} | {safe_reason}")
            else:
                abnormal_report.append(f"⚠️警告：{uid} | {safe_reason}")

        # 生成CSV报告
        self._generate_batch_report_csv(abnormal_report, success, warn, fail, total)
    ...
    def _detect_single_file_batch(self, file_path, bin_path, max_details, min_money, min_num, export_full, uid_total_cost):
        ...
        if match:
            ...
            server_uid = f"ERR_{hash(file_path) % 10000}"
            file_index = "0"
        ...
        total_cost_all = uid_total_cost[server_uid]
    ...
    def _generate_batch_report_csv(self, abnormal_report, success, warn, fail, total):
        """生成批量检测报告，格式为CSV"""
        report_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        csv_rows = []
        csv_rows.append(['uid', 'status', '异常详情'])
        valid_count = 0
        ...
        for line in abnormal_report:
            ...
            if ' | ' in content:
                ...
                if len(detail) > 500:
                    detail = detail[:500] + "..."
                csv_rows.append([uid, status, detail])
                valid_count += 1
            else:
        ...
        if self._get_bool('auto_open_batch_report', False) and valid_count > 0:
            self._open_dir(OUTPUT_DIR)
