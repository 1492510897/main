...
class DetectionEngine:
    ...
    def check_pay_xml(file_path, bin_path, max_details=10, min_money_details=500, min_number_details=500, export_full=False):
        ...
        try:
            ...
            msg_lines = [f"💰 估算存档消费：{total_cost} 黄金"]
            status = 'pass'
    ...
    def check_vip_pay_union(file_path, pay_cost):
        ...
        if limit_amount2 is None or limit_amount2 == 0: return {'status': 'pass', 'msg': f'ℹ️ VIP 等级 {vip_level} 不在对照表中或额度为 0。'}
        ...
        if ratio2 > 1.0:
            status = 'fail'
            msg_lines.append(f"🚨 作弊原因:VIP{vip_level}\n实际消耗：{pay_cost}")
        elif ratio1 < 0.3:
            status = 'warn'
            msg_lines.append(f"⚠️ 警告原因:VIP{vip_level}\n实际消耗：{limit_amount1}\n可能出现覆盖存档")
        else:
    ...
    def check_vip_xml(file_path):
        try:
            ...
            return {'status': 'pass', 'msg': f'✅ VIP {vip_level}'}
        except Exception as e:
    ...
    def check_cheat_stream(file_path, check_flag=True, check_reason=True):
        try:
            ...
            if check_flag:
                if flag_node_found:
                    if flag_value == "false":
                        results.append({'title': "🚫 IsZuobiB 检测", 'status': 'pass', 'msg': "✅ 正常"})
                    else:
                        cheat_found = True
                ...
                if reason_content is not None:
                    ...
                    if clean_reason == "":
                        if not (flag_node_found and flag_value == "false"):
                            ...
                            results.append({'title': "📝 ZuobiReason 分析", 'status': 'pass', 'msg': "✅ 无作弊记录"})
                    else:
                        cheat_found = True
                    ...
                    if flag_node_found and flag_value != "false":
                        results.append({'title': "📝 ZuobiReason 分析", 'status': 'warn', 'msg': "⚠️ 已标记作弊但未找到节点"})
                    else:
                        results.append({'title': "📝 ZuobiReason 分析", 'status': 'pass', 'msg': "✅ 未发现作弊记录"})
...
class AppDetector:
    def __init__(self, root):
        self.root = root
        self.root.title("数据检测工具 v7.2.3|BY. 观星者")
    ...
    def _build_ui(self):
        ...
        tk.Label(nav_frame, text="数据检测工具 v7.2.3", bg=self.COLOR_NAV_BG, fg="white", font=("Microsoft YaHei", 12, "bold")).place(relx=0.7, y=0)
...
"""
    ...
    def _do_batch_detect(self, folder_path):
        ...
        with ThreadPoolExecutor(max_workers=thread_count) as executor:
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    self.log(f" 文件UID:{os.path.basename(fp)}\n存档实际UID: {res['uid']}", status)
        ...
        vip_min = {10:0,100: 1, 200: 2, 500: 3, 1000: 4, 2000: 5, 5000: 6, 8000: 7, 10000: 8, 20000: 9, 50000: 10}
        vip_max = {100: 0, 200: 1, 500: 0, 1000: 3, 2000: 4, 5000: 5, 8000: 6, 10000: 7, 20000: 8, 50000: 9, 100000: 10}
        vip_to_min = {v: k for k, v in vip_min.items()}
        vip_to_max = {v: k for k, v in vip_max.items()}
        ...
        for key, reason in SLOT_BAN_CACHE.ban_cache.items():
            ...
            if match:
                ...
                detail_filename = f"{fname}_异常详情.txt"
                detail_path = os.path.join(detail_dir, detail_filename)
                with open(detail_path, "w", encoding="utf-8") as f:
                    f.write(f"🚨 检测结果：严重异常 (封禁)\n")
                    f.write(f"文件名：{fname}\n")
                    f.write(f"UID：{uid}\n")
                
                abnormal_report.append(f"封禁存档：{fname}")
        ...
        for file_path, res in file_result_map.items():
            ...
            uid_str = res['uid']
            real_uid = uid_str.split('_')[0] if '_' in uid_str else 'unknown'
            
            detail_lines = []
            file_reason = "正常: 未发现异常" # 默认值改为“正常”，而不是“NULL”
            for item in res['results']:
                if isinstance(item, dict):
                    ...
                    # --- 核心补全逻辑：提取违规原因 ---
                    # 只有当状态是 fail 或 warn 时才提取原因
                    if item['status'] in ['fail', 'warn']:
                        # 优先级 1：如果还没有原因，直接赋值
                        if file_reason == "正常: 未发现异常":
                            file_reason = f"[{item['status'].upper()}] {title}: {msg.split('.')[0]}..."
            for item in res['results']:
                if isinstance(item, dict):
                    title = item.get('title', '检测项')
                    msg = item.get('msg', '')
                    ...
                    if item['status'] in ['fail', 'warn'] and file_reason == "NULL":
                        if "作弊" in msg:
                            file_reason = f"作弊标记: {msg}"
                        elif "VIP" in msg:
                            file_reason = f"VIP异常: {msg}"
            ...
            try:
                ...
                pass # 忽略单个文件读取 VIP 失败的错误，避免日志刷屏

            if vip_level >= 0 and total_cost > 0:
                limit_min = vip_to_min.get(vip_level, 0)
                limit_max = vip_to_max.get(vip_level, 0)

                if limit_max > 0:
                    ratio_min = (total_cost+10) / limit_min
                    ratio_max = total_cost / limit_max
                    if ratio_max > 1.0:
                        union_msg = f"VIP：{vip_level}消耗金额：{total_cost}存在作弊情况"
                        file_status = 'fail'
                        file_reason = union_msg
                    elif ratio_min < 0.7:
                        union_msg = f"VIP：{vip_level}消耗金额：{total_cost}有覆盖存档可能"
                        if file_status == 'pass':
                            file_status = 'warn'
                            file_reason = union_msg


            if real_uid not in uid_report_map:
                uid_report_map[real_uid] = {
                    'uid': real_uid,
                    'status': file_status,
                    'reason': file_reason,
            ...
            current = uid_report_map[real_uid]
            current['files'].append(fname)
            
            if file_status == 'fail':
                current['status'] = 'fail'
                current['reason'] = file_reason 
                current['details'].extend(detail_lines)
            elif file_status == 'warn' and current['status'] != 'fail':
                current['status'] = 'warn'
                if current['reason'] == "NULL":
                     current['reason'] = file_reason
                current['details'].extend(detail_lines)

        for uid, data in uid_report_map.items():
            if data['status'] in ['fail', 'warn']:
                detail_filename = f"{uid}.txt" 
                detail_path = os.path.join(detail_dir, detail_filename)
                
                content_lines = []
                content_lines.append(f"📄 文件：{', '.join(data['files'])}")
                content_lines.append(f"🆔 UID：{data['uid']}")
                content_lines.append(f"💰 此UID下消费：{data['total_cost']}")
                content_lines.append(f"🔍 检测结果：{data['status'].upper()}")
                content_lines.append(f"📝 原因：{data['reason']}")
                
                if data['details']:
                    content_lines.append("\n" + "="*50 + " 详细日志 " + "="*50 + "\n")
                    content_lines.extend(data['details'])

                with open(detail_path, "w", encoding="utf-8") as f:
                    f.write("\n".join(content_lines))

                is_union_issue = ("UID下消耗" in data['reason']) or ("覆盖存档" in data['reason'])
                
                if is_union_issue:
                    display_id = uid
                else:
                    first_file = data['files'][0]
                    file_index = first_file.replace('.xml', '').split('_')[-1]
                    display_id = f"{uid}_{file_index}"

                safe_reason = data['reason'].replace('\n', ' ')
                if len(safe_reason) > 60:
                    safe_reason = safe_reason[:60] + "..."

                if data['status'] == 'fail':
                    abnormal_report.append(f"❌异常：{display_id} |  {safe_reason}")
                elif data['status'] == 'warn':
                    abnormal_report.append(f"⚠️警告：{display_id} |  {safe_reason}")

        self._generate_batch_report(abnormal_report, success, warn, fail, total)
        self.log(f"✅ 批量检测结束\n正常:{success} 警告:{warn} 异常:{fail}", "PASS")

    def _detect_single_file_batch(self, file_path, bin_path, max_details, min_money, min_num, export_full, uid_total_cost):
        fname = os.path.basename(file_path)
        
        # 1. 获取【本地 XML UID】(用于对比和显示)
        uid_res = DetectionEngine.check_uid_md5_advanced(file_path)
        local_uid_full = uid_res.get('uid', '') if uid_res else ''
        
        match = re.match(r"^(\d+)_(\d+)\.xml$", fname, re.IGNORECASE)
        if match:
            server_uid = match.group(1) # 用于字典 Key
            file_index = match.group(2)
        else:
            server_uid = f"ERR_FILE_{hash(file_path)}"
            file_index = "0"
        
        if local_uid_full and '_' in local_uid_full:
            local_uid_part = local_uid_full.split('_')[0]
            if local_uid_part != server_uid and not server_uid.startswith("ERR_"):
                if isinstance(uid_res, dict):
                    original_msg = uid_res.get('msg', '')
                    uid_res['msg'] = f"❌ 【异常：UID 不匹配】\n文件名UID:{server_uid}_{file_index}\n数据UID:{local_uid_full}\n" + original_msg
                    uid_res['status'] = 'fail' 
        elif not local_uid_full:
            local_uid_full = f"{server_uid}_{file_index} "
        ...
        # 5. 【核心修复】使用 Server_UID (文件名 UID) 进行金额累加
        current_cost = pay_res.get('total_cost', 0)
        
        # 无论 XML 里写什么，都累加到文件名对应的 UID 上
        uid_total_cost[server_uid] += current_cost 
        total_cost_all = uid_total_cost[server_uid] # 获取该 UID 的总累计消费

        union_res = DetectionEngine.check_vip_pay_union(file_path, total_cost_all)

        # 7. 构建封禁结果
        ban_result = {"status": "pass", "title": "封禁", "msg": "正常"}
        if ban_reason:
            ban_result = {"status": "fail", "title": "封禁", "msg": ban_reason}
        ...
        for r in all_checks:
            if isinstance(r, dict) and r.get('status') == 'fail':
                overall = 'fail'
                break
        if overall == 'pass': # 只有在不是 Fail 的情况下才检查 Warn
            for r in all_checks:
                if isinstance(r, dict) and r.get('status') == 'warn':
                    overall = 'warn'
                    break

        report_uid = f"{server_uid}_{file_index}"
    ...
    def _generate_batch_report(self, abnormal_report, success, warn, fail, total):
        report_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        report_name = f"批量检测报告_{report_time}.txt"
        report_path = os.path.join(OUTPUT_DIR, report_name)
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"批量检测总报告\n")
            f.write(f"检测时间：{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"总计：{total} | 异常(Fail)：{fail} | 警告(Warn)：{warn} | 正常：{success}\n")
            f.write("=" * 60 + "\n\n")

            if abnormal_report:
                f.write("📋 封禁情况:\n")
                f.write("\n".join(abnormal_report))
            else:
                f.write("🎉 恭喜，本次检测未发现异常文件！\n")
