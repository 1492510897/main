...
class DetectionEngine:
    ...
    def check_pay_xml(file_path, bin_path, max_details=10, min_money_details=500, min_number_details=500, export_full=False):
        ...
        if price_map is None:
            return {'title':'金币消费检测','status': 'fail', 'msg': f"❌ 错误：无法读取或解密价格表文件。\n路径：{bin_path}\n(文件不存在或格式错误)", 'total_cost': 0}
        if not price_map:
            return {'title':'金币消费检测','status': 'warn', 'msg': "⚠️ 价格表为空，跳过消费计算。", 'total_cost': 0}
        ...
        try:
            ...
            if root is None: return {'title':'金币消费检测','status': 'fail', 'msg': "❌ XML 文件严重损坏，无法解析支付节点。", 'total_cost': 0}
            pay_node = root.find('.//s[@name="pay"]')
            if pay_node is None: return {'title':'金币消费检测','status': 'warn', 'msg': "⚠️ 未找到 <pay> 节点", 'total_cost': 0}
            obj_node = pay_node.find('.//s[@name="obj"]')
            if obj_node is None: return {'title':'金币消费检测','status': 'warn', 'msg': "⚠️ <pay> 节点下未找到 <obj> 子节点", 'total_cost': 0}
            ...
            return {'title':'金币消费检测','status': status, 'msg': "\n".join(msg_lines), 'total_cost': total_cost}
        except Exception as e:
            return {'title':'金币消费检测','status': 'fail', 'msg': f"解析错误：{str(e)}", 'total_cost': 0}
    ...
    def check_uid_md5_advanced(file_path):
        try:
            ...
            # 5. 保底 MD5 匹配（使用 un2/uu2 作为 uid）
            if target_md5:
                for index, fallback_md5 in enumerate(FALLBACK_MD5_LIST):
                    if target_md5 == fallback_md5.lower():
                        if raw_uid:
                            # 保底签名匹配，使用 un2/uu2 作为 uid，index 从保底列表获取
                            return {
                                'title':'UID 验证',
                                'status': 'warn',
                                'msg': f"⚠️ 保底签名匹配\nUID：{raw_uid}\nIndex：{index}",
                                'uid': f"{raw_uid}_{index}"  # 返回完整的 uid_index 格式
                            }
                        else:
                            return {
                                'title':'UID 验证',
                                ...
                                'uid': f"0_{index}"  # 使用 0_{index} 作为保底
                            }
            
            # 6. 没有 un2 和 uu2（两者都找不到）-> 使用文件名
            if not raw_uid:
                fname = os.path.basename(file_path)
                match = re.match(r"^(\d+)_(\d+)\.xml$", fname)
                if match:
                    uid_from_filename = f"{match.group(1)}_{match.group(2)}"
                    return {
                        'title':'UID 验证',
                        'status': 'fail',
                        'msg': f'❌ 找不到 un2 和 uu2，使用文件名标识\nUID：{uid_from_filename}',
                        'uid': uid_from_filename
                    }
                return {
                    'title':'UID 验证',
            ...
            # 7. 有 un2/uu2，但没有 MD5 签名 -> 使用 un2/uu2
            if not target_md5:
                return {
                    'title':'UID 验证',
                    'status': 'fail',
                    'msg': f'❌ 找到原始 UID {raw_uid}，但文件中无数字签名（uidMd5 缺失）',
                    'uid': raw_uid
                }
            
            # 8. 核心验证：用 raw_uid + index (0~7) 计算 md5，与 target_md5 比对
            for index in range(8):
                ...
                if calc_md5 == target_md5:
                    return {
                        ...
                        'status': 'pass',
                        'msg': f"✅ UID 验证通过\nUID：{raw_uid}\nIndex：{index}",
                        'uid': f"{raw_uid}_{index}"
                    }
            
            # 9. 计算值均不匹配 -> 使用 un2/uu2
            return {
                'title':'UID 验证',
                'status': 'fail',
                'msg': f"❌ UID 签名验证失败\n原始 UID：{raw_uid}\n期望 MD5：{target_md5}\n（计算值均不匹配）",
...
class AppDetector:
    ...
    def _save_config(self, is_init=False):
        ...
        self.config['Security']['version'] = '2.0.4'
    ...
    def _build_ui(self):
        ...
        tk.Label(nav_frame, text="数据检测工具 2.0.4", bg=self.COLOR_NAV_BG, fg="white", font=("Microsoft YaHei", 12, "bold")).place(relx=0.8, y=0)
...
"""
    ...
    def _do_batch_detect(self, folder_path):
        ...
        for file_path, res in file_result_map.items():
            ...
            # 获取 display_id
            real_uid_full = res.get('uid', '')
            if real_uid_full and '_' in real_uid_full:
                display_id = real_uid_full
            else:
                display_id = fname.replace('.xml', '')

            detail_lines = []
            file_reason = "正常: 未发现异常"
            combined_fail_msgs = []  # 收集所有失败/警告的原始消息
            combined_warn_msgs = []
            ...
            # 获取消费金额
            uid_for_cost = display_id.split('_')[0] if '_' in display_id else display_id
            total_cost = uid_total_cost.get(uid_for_cost, 0)
            
            # 生成异常详情文件
            detail_filename = f"{display_id}_异常详情.txt"
            detail_path = os.path.join(detail_dir, detail_filename)
            
            content_lines = []
            content_lines.append(f"📄 文件：{fname}")
            content_lines.append(f"🆔 UID：{display_id}")
            ...
            # 构建 CSV 行（直接使用 file_reason，不添加额外前缀）
    ...
    def _detect_single_file_batch(self, file_path, bin_path, max_details, min_money, min_num, export_full, uid_total_cost):
        ...
        uid_res = DetectionEngine.check_uid_md5_advanced(file_path)
        real_uid_full = uid_res.get('uid', '') if uid_res else ''
        ...
        if real_uid_full and '_' in real_uid_full:
            # 验证通过，有完整 uid_index
            final_uid = real_uid_full
            uid_for_cost = real_uid_full.split('_')[0]
            detail_filename = f"{final_uid}_异常详情.txt"
        elif raw_un2_uu2:
            # 有 un2/uu2，但没有完整验证
            final_uid = raw_un2_uu2
            uid_for_cost = raw_un2_uu2
            detail_filename = f"{final_uid}_异常详情.txt"
        else:
            # 无法获取 UID，直接使用文件名（不含扩展名）
            final_uid = fname.replace('.xml', '')
            uid_for_cost = final_uid
            detail_filename = f"{final_uid}_异常详情.txt"
    ...
    def _generate_batch_report(self, abnormal_report, success, warn, fail, total):
        ...
        csv_rows = []
        csv_rows.append(['uid_index', 'status', '异常详情'])
        valid_count = 0
        ...
        for line in abnormal_report:
            ...
            if ' | ' in content:
                ...
                uid_index = parts[0].strip()
                detail = parts[1].strip() if len(parts) > 1 else ''
                # 清理详情中的特殊字符
                detail = detail.replace('\n', ' ').replace('\r', ' ').replace(',', '，')
                if len(detail) > 500:
                    detail = detail[:500] + "..."
                csv_rows.append([uid_index, status, detail])
                valid_count += 1
            else:
                # 没有详情的情况
