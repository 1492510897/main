...
"""
    ...
    def _do_batch_detect(self, folder_path):
        ...
        for file_path, res in file_result_map.items():
            ...
            content_lines.append(f"📁 理论UID_Index：{file_uid_index if file_uid_index else '无'}")
            content_lines.append(f"📄 实际UID_Index：{inner_uid_index if inner_uid_index else '无'}")
            if file_name:
                content_lines.append(f"📛 Name：{file_name}")
            ...
            # CSV 显示：理论uid_index, 实际uid_index, Name, 状态, 异常详情
    ...
    def _generate_batch_report(self, abnormal_report, success, warn, fail, total):
        ...
        csv_rows = []
        # 添加列头 - 修改为理论uid_index, 实际uid_index, Name, 状态, 异常详情
        csv_rows.append(['理论uid_index', '实际uid_index', 'Name', '状态', '异常详情'])
        valid_count = 0
