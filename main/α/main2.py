...
class DetectionEngine:
    ...
    def check_uid_md5_advanced(file_path):
        try:
            ...
            if match_full:
                filename_uid = match_full.group(1)
                filename_index = match_full.group(2)
                filename_name = match_full.group(3)
            else:
                # 匹配简单的 uid_index 格式
                match_simple = re.match(r"^(\d+)_(\d+)$", fname_without_ext)
                if match_simple:
                    filename_uid = match_simple.group(1)
                    filename_index = match_simple.group(2)
                    filename_name = None
                    ...
                    if target_md5 == fallback_md5.lower():
                        if raw_uid:
                            return {
                                ...
                                'index': str(index),
                                'name': filename_name,
                                'uid_index': f"{raw_uid}_{index}"
                            }
                        else:
                            return {
                                'title':'UID 验证',
                                'status': 'warn',
                                'msg': f"⚠️ 保底签名匹配，但无uid，使用保底索引 {index}",
                                'uid': '0',
                                'index': str(index),
                                'name': filename_name,
                                'uid_index': f"0_{index}"
                            }
            
            # 7. 没有 un2 和 uu2（两者都找不到）-> 使用文件名信息
            if not raw_uid:
                if filename_uid and filename_index:
                    uid_index_str = f"{filename_uid}_{filename_index}"
                    name_str = f"_{filename_name}" if filename_name else ""
                    return {
                        'title':'UID 验证',
                        'status': 'fail',
                        'msg': f'❌ 找不到 un2 和 uu2，使用文件名标识\nUID_INDEX：{uid_index_str}\nNAME：{filename_name if filename_name else "无"}',
                        'uid': filename_uid,
            ...
            # 8. 有 un2/uu2，但没有 MD5 签名 -> 使用 un2/uu2 + 文件名name（如果有）
            if not target_md5:
                uid_index_str = f"{raw_uid}_0"
                name_str = f"_{filename_name}" if filename_name else ""
                return {
                    'title':'UID 验证',
                    'status': 'fail',
                    'msg': f'❌ 找到原始 UID {raw_uid}，但文件中无数字签名（uidMd5 缺失）{name_str}',
                    'uid': raw_uid,
                    'index': '0',
                    'name': filename_name,
                    'uid_index': uid_index_str
            ...
            # 9. 核心验证：用 raw_uid + index (0~7) 计算 md5，与 target_md5 比对
            for index in range(8):
                test_str = f"{raw_uid}_{index}"
                calc_md5 = hashlib.md5(test_str.encode('utf-8')).hexdigest().lower()
                
                if calc_md5 == target_md5:
                    uid_index_str = f"{raw_uid}_{index}"
                    # 修改这里：不要在消息中把 name 拼接到 Index 后面
                    return {
                        'title':'UID 验证',
                        'status': 'pass',
                        'msg': f"✅ UID 验证通过\nUID：{raw_uid}\nIndex：{index}",
            ...
            # 10. 计算值均不匹配 -> 使用 un2/uu2 + 文件名name
            uid_index_str = raw_uid
            name_str = f"_{filename_name}" if filename_name else ""
            return {
                'title':'UID 验证',
                'status': 'fail',
...
"""
    ...
    def _do_batch_detect(self, folder_path):
        ...
        for file_path, res in file_result_map.items():
            ...
            # 只处理异常和警告的文件
            if file_status not in ['fail', 'warn', 'pass']:
                continue
            
            # 获取分离的数据
            uid_value = res.get('uid', '')
            index_value = res.get('index', '')
            name_value = res.get('name', '')
            uid_index_value = res.get('uid_index', '')
            
            # 构建显示标识
            if uid_index_value and name_value:
                display_id = f"{uid_index_value}_{name_value}"
            elif uid_index_value:
                display_id = uid_index_value
            elif uid_value:
                display_id = uid_value
            else:
                display_id = fname.replace('.xml', '')
            
            # 获取消费金额
            uid_for_cost = res.get('raw_uid', uid_value)
            if not uid_for_cost or not str(uid_for_cost).isdigit():
                match_uid = re.match(r"^(\d+)", str(uid_for_cost))
                if match_uid:
                    uid_for_cost = match_uid.group(1)
                else:
                    uid_for_cost = uid_value
            ...
            for item in res['results']:
                if isinstance(item, dict):
                    ...
                    elif status == 'warn' and not combined_fail_msgs:
                        short_msg = msg.split('\n')[0] if msg else ""
                        if len(short_msg) > 100:
                            short_msg = short_msg[:100] + "..."
            ...
            # 生成异常详情文件
            safe_display_id = re.sub(r'[<>:"/\\|?*]', '_', display_id)
            detail_filename = f"{safe_display_id}_异常详情.txt"
            detail_path = os.path.join(detail_dir, detail_filename)
            
            content_lines = []
            content_lines.append(f"📄 原始文件：{fname}")
            content_lines.append(f"🆔 UID：{uid_value}")
            content_lines.append(f"🔢 Index：{index_value}")
            if name_value:
                content_lines.append(f"📛 Name：{name_value}")
            content_lines.append(f"🔗 UID_INDEX：{uid_index_value}")
            content_lines.append(f"💰 消费金额：{total_cost}")
            content_lines.append(f"🔍 检测结果：{file_status.upper()}")
            content_lines.append(f"📝 原因：{file_reason}")
            ...
            # CSV 中显示完整信息
            csv_display = f"{uid_index_value}_{name_value}" if name_value else uid_index_value
            
            if file_status == 'fail':
                abnormal_report.append(f"❌异常：{csv_display} | {safe_reason}")
            elif file_status == 'warn':
                abnormal_report.append(f"⚠️警告：{csv_display} | {safe_reason}")
    ...
    def _detect_single_file_batch(self, file_path, bin_path, max_details, min_money, min_num, export_full, uid_total_cost):
        ...
        # 提取分离的数据
        uid_value = uid_res.get('uid', '') if uid_res else ''
        index_value = uid_res.get('index', '') if uid_res else ''
        name_value = uid_res.get('name', '') if uid_res else ''
        uid_index_value = uid_res.get('uid_index', '') if uid_res else ''
        
        # 2. 直接从 XML 读取原始 un2/uu2
        raw_un2_uu2 = None
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
            # 优先提取 un2
            un2_match = re.search(r'<s[^>]*name=["\']un2["\'][^>]*>([^<]+)</s>', content, re.IGNORECASE)
            if un2_match:
                raw_un2_uu2 = un2_match.group(1).strip()
            else:
                # 提取 uu2
                uu2_match = re.search(r'<s[^>]*name=["\']uu2["\'][^>]*>([^<]+)</s>', content, re.IGNORECASE)
                if uu2_match:
                    raw_un2_uu2 = uu2_match.group(1).strip()
        except Exception:
            pass
        
        # 3. 确定最终的各个部分
        if uid_value and index_value:
            # 有完整的 uid 和 index
            final_uid = uid_value
            final_index = index_value
            final_uid_index = f"{final_uid}_{final_index}"
            final_name = name_value
        elif raw_un2_uu2:
            # 有 un2/uu2，但没有完整验证
            final_uid = raw_un2_uu2
            # 尝试从文件名提取 index 和 name
            match_full = re.match(r"^(\d+)_(\d+)_(.+)$", fname_without_ext)
            if match_full:
                final_index = match_full.group(2)
                final_name = match_full.group(3)
            else:
                match_simple = re.match(r"^(\d+)_(\d+)$", fname_without_ext)
                if match_simple:
                    final_index = match_simple.group(2)
                    final_name = None
                else:
                    final_index = '0'
                    final_name = None
            ...
            if match_full:
                ...
                match_simple = re.match(r"^(\d+)_(\d+)$", fname_without_ext)
                if match_simple:
                    final_uid = match_simple.group(1)
                    final_index = match_simple.group(2)
                    final_name = None
                    final_uid_index = f"{final_uid}_{final_index}"
                else:
                    # 完全无法解析
                    final_uid = fname_without_ext
                    final_index = '0'
                    final_name = None
                    final_uid_index = final_uid
        
        # 用于消费统计的纯数字UID
        uid_for_cost = final_uid if final_uid.isdigit() else final_uid.split('_')[0] if '_' in final_uid else final_uid
    ...
    def _generate_batch_report(self, abnormal_report, success, warn, fail, total):
        ...
        csv_rows = []
        # 添加列头
        csv_rows.append(['UID', 'Index', 'Name', '状态', '异常详情'])
        valid_count = 0
        ...
        for line in abnormal_report:
            ...
            if ' | ' in content:
                ...
                display_id = parts[0].strip()  # 格式如: 1043321665_3_落尘223 lv.99XinLing
                detail = parts[1].strip() if len(parts) > 1 else ''
                
                # 解析 display_id 为 UID, Index, Name
                uid = ''
                index = ''
                name = ''
                
                # 匹配格式: uid_index_name
                match = re.match(r"^(\d+)_(\d+)_(.+)$", display_id)
                if match:
                    uid = match.group(1)
                    index = match.group(2)
                    name = match.group(3)
                else:
                    # 尝试匹配简单的 uid_index 格式
                    match_simple = re.match(r"^(\d+)_(\d+)$", display_id)
                    if match_simple:
                        uid = match_simple.group(1)
                ...
                csv_rows.append([uid, index, name, status, detail])
                valid_count += 1
            else:
                # 没有详情的情况，尝试解析标识
                display_id = content
                ...
                if match:
                    ...
                    if match_simple:
                        ...
                        index = ''
                        name = ''
                csv_rows.append([uid, index, name, status, ''])
                valid_count += 1
